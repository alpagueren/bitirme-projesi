...
export default function EtkinlikListesi() {
  const useStyles = makeStyles((theme) => ({
    ...
    useEffect(() => {
      const etkinlikReference = firebase.database().ref('fakulteetkinlik');
    ...
    return (
        <div styles={{}}>
            {etkinlikListesi ? etkinlikListesi.map((fakulteetkinlik) =>
            <List component="nav" className={classes.root} aria-label="mailbox folders">
            <ListItem button>
              <ListItemText primary={fakulteetkinlik.baslik} />
              Icerik:  {fakulteetkinlik.icerik}
              <br/>
              <br/>
              Konum: {fakulteetkinlik.konum}
              <br/>
              Zamanı:  {fakulteetkinlik.etkinlikZamani}
            </ListItem>                    
          </List>
